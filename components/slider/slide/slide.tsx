...
import { SlidesType } from "components/slider/multiple";
import Image from "next/image";

const Slide = (data: SlidesType["slides"][0]) => {
  const { colors, discountPercent, image, title, currentPrice, oldPrice, id } =
    data;
  ...
  return (
    <div
      role="button"
      className="card p-0 slide shadow-sm"
    ...
    >
      <div className="slide__title">
        <Image
          ...
          alt=""
        />
        <span className="slide__title-discount-percent">
          {discountPercent}
          <span className="percent-sign">%</span>
        </span>
        <div className="slide__title-wave">
          <Image
            className=" overflow-visible position-absolute"
            src={Wave}
            layout={"fill"}
            alt=""
          />
      ...
      <div className="slide__body">
        ...
        <ul className="d-flex">
          {colors.map((color: string, index: number) => (
            <li
              key={index}
              className="slide__body-colors-item"
              style={{ backgroundColor: color }}
            />
          ))}
        </ul>
        <div className="d-flex justify-content-end slide__body-price align-items-center">
          <span className="fw-bolder ms-2 color-mischka text-decoration-line-through slide__body-price-current">
            {currentPrice}
          </span>
