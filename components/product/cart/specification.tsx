...
const Specification = () => {
  ...
  return (
    <div className="product__cart">
          ...
          <ul>
            {colors.map((item, index) => (
              <li
                ...
                role="button"
                className={`product__cart-color-item d-inline-block opacity-40
                                                      ${
                                                        color === item
                                                          ? "product__cart-color-item--active opacity-100"
                                                          : ""
                                                      }`}
                key={index}
            ...
            <ul>
              {sizes.map((item, index) => (
                <li
                  ...
                  role="button"
                  className={`product__cart-size-item d-inline-block  opacity-40
                             ${size === item ? "opacity-100 text-black" : ""}`}
                  key={index}
                >
                  {item}
                </li>
      ...
      <div>
        <span className="color-mischka">موجودی :</span>{" "}
        <span className="fw-bold">5 عدد</span>
      </div>
      <div>
        <span className="color-mischka">تخفیف :</span>{" "}
        <span className="fw-bold">%20</span>
      </div>
      <div>
        <span className="color-mischka"> قیمت :</span>{" "}
        <span className="text-decoration-line-through fw-bold font-15 color-mischka">
          325000
        </span>{" "}
      ...
      <div>
        ...
        <div className="product__cart-count d-flex justify-content-between align-items-center">
          <button
            onClick={handlePlus}
            className={`product__cart-count-plus ${
              count > 0 ? "product__cart-count-plus--active" : ""
            }`}
          >
            <FaPlus />
          </button>
          ...
          <button
            onClick={handleMinus}
            disabled={count === 1}
            className={`product__cart-count-minus ${
              count > 0 ? "product__cart-count-minus--active" : ""
            }`}
          >
            <FaMinus />
          </button>
