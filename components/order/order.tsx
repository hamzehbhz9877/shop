...
const option = (type: string) =>
  ({
    جاری: "order__current",
    "تحویل شده": "order__delivered",
    "مرجوع شده": "order__returned",
    "لغو شده": "order__canceled",
  }[type] ?? "");

const Order = ({ type, className, ...rest }: OrderType) => {
  return (
    <div
      className={`card order flex-column flex-sm-row  shadow-sm ${
        className ?? ""
      }`}
      {...rest}
    >
      <div>
        ...
        <div className="order__spec d-flex flex-wrap gap-3 text-center">
          <span className="color-mischka ">5 خرداد 1401</span>
          <div className="line-splitter m-0 d-none d-sm-block" />
